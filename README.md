...
회의록 품질을 높이기 위해 `dictionary/` 폴더의 용어 사전을 사용합니다.
...
`terms.xlsx`는 아래 구조를 기본으로 읽습니다.

```text
표준 용어 | 자주 나오는 표현 | 회의록에서 정리할 표현
```

서버 시작 시 XLSX와 JSON을 메모리에 로드합니다. 파일을 교체한 경우 서버를 재시작하면 새 용어가 반영됩니다. JSON에는 특히 중요한 내부 용어를 직접 넣을 수 있습니다.

용어 사전은 세 단계에 사용됩니다.

```text
1. STT/transcript 후처리: 유사 표현과 오타를 표준 용어로 보정
2. GPT 요약 프롬프트: 회사 용어와 설명을 함께 전달
3. 요약 후처리: Markdown 안의 용어 표기를 다시 표준화
```
...
다솜이 / 다솜 로봇 / 다솜서비스 → 다솜
아바딘 / 아바딘이 → Avadin
wake word / 웨이크워드 → 호출어 변경
Guardian App → 보호자앱
```

요약 프롬프트도 단순 bullet 압축이 아니라 “핵심 주제 → 현재 문제 → 왜 중요한지 → 결론” 흐름으로 회의 의미를 설명하도록 개선했습니다. 중요한 논의는 `[매우 중요]`, `[핵심]`, `[리스크]`처럼 강조하고, 회사 내부 용어와 제품명은 임의 축약하지 않도록 지시합니다.
